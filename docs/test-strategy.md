...
| NFR7 | Compose-up < 60 s | `scripts/test-compose-up-time.sh` — invokes `docker compose up --wait --wait-timeout 60`, times the wall-clock, fails if exceeded. Run with `npm run test:nfr7` | Wire into CI when added |
