...
This log is my retrospective covering how I used Claude to take the project from a single-paragraph PRD to a tested, containerized, CI-gated v1. It's a project artifact. My goal here is to leave behind an honest record of what Claude did well, what it got wrong, and where my own judgement was needed.
...
| **QA suite** | 25 test files / 129 cases — Vitest (backend + frontend), Supertest, RTL, MSW, Playwright, axe-playwright; coverage with v8 | All cases mapped to PRD requirement IDs in `docs/test-strategy.md` §11. Backend coverage gate enforced at ≥ 80% lines / functions / statements, ≥ 75% branches. |
...
| **Spec / impl audits** | Audit findings logged inline as D-A1 through D-A7 (see §4 "Architecture / spec-vs-code audits") | Claude compared what the architecture doc said against what the code actually did. The biggest finding (D-A2: RepoContext "opaque options" claim vs concrete-params reality) was fixed by *changing the code to match the doc*, not the reverse — I explicitly chose that direction. |
...
- **Bundled approval after a numbered findings list.** When Claude produced a numbered list of findings (spec audit, security review, coverage gaps, bug review), I could approve a strategic scope with one short reply — *"fix all 7"*, *"batch a"*, *"do all 3"*. That kept the `pending → in_progress → completed` TodoList rhythm intact without needing approval per item. The two flavours that worked: **all-items** ("fix all N") for an exhaustive batch where everything was in scope, and **scope-selection** ("batch a" out of A / B / C tiers) when the work split naturally into important / nice-to-have / polish layers and Claude had laid out the tiers up-front. The latter is the one I'd reach for first next time — it preserves my agency over scope while still moving fast.
- **"Where do I have gaps in X?"** — spec-vs-implementation audit prompts. *"Where do I have gaps in my integration test coverage?"*, *"According to my original specifications, are there any more test gaps in coverage on the frontend?"* These forced Claude to ground answers in the actual spec / story files (`docs/stories/E3.*.md`, the PRD's FR/NFR list) rather than ad-hoc opinion, and produced batches of work mapped 1:1 against documented acceptance criteria. Different in shape from *"do I have a test for X?"* because it asks for a **categorised list of missing items** — which surfaces gaps Claude wouldn't otherwise have flagged.
...
- **"Review X, Y, Z for accuracy and consistency."** — meta-level audit prompts. *"Are my accessibility tests and security review still up to date?"*, *"Does my README need updating?"*, *"Review my documentation for accuracy and consistency."* Not a request for new work — a request to **verify** previous work. These triggered the count-propagation audits, date-stamp checks, and cross-reference sweeps that caught real drift: the off-by-one "12 axe scans" claim, the NFR10 mis-attribution in `test-strategy.md` §5, the stale 89.03% coverage figure after Batch A, the broken `docs/qa/spec-audit.md` reference. Asking Claude to verify its own claims periodically catches drift that Claude won't surface on its own.
- **Pasting canonical URLs as comparison anchors.** When asking Claude to compare Playwright MCP / Chrome DevTools MCP to the existing setup, I pasted *https://playwright.dev/docs/getting-started-mcp* and *https://developer.chrome.com/blog/chrome-devtools-mcp* rather than asking Claude to recite benefits from training data. That forced a `web_fetch` against the official docs and grounded the comparison in current facts, not memory. Small move; meaningful quality lift for any "compare X to Y" question where one of them might have changed since training cutoff.
...
| **a11y** | `axe-playwright` baked into specs — 11 axe scans across 11 UI states, asserted with `checkA11y` (plus 1 keyboard-reachability test in the same suite). | The MCP itself doesn't run axe, but its aria-snapshots make accessibility issues visible to the agent at the same level axe would catch them. Complement, not replacement. |
...
Claude generated every test file in this repo — all 25 files, all 129 cases (112 written against the original story specs, 3 added during the B1/B2/B5 bug-fix round, 6 added during the integration-coverage gap-closure round, 1 added to close the NFR10 traceability lie, 6 added during a frontend gap-closure round that closed E3.S1 I1/I3/I4 + U1/U2 and E3.S5 I1, and 1 more added during a post-audit gap closure to pin FR5's `is-completed` class flip at the unit level — previously only verified by the e2e axe scan). The pattern: read the corresponding story's *Test Strategy* section, then emit a unit / integration / e2e spec that mirrored the Given/When/Then list 1:1, then map each case back to its PRD requirement ID for traceability (`docs/test-strategy.md` §11).

### What worked well

- **Story-first authoring.** Every story in `docs/stories/` already enumerated test scenarios. Generating the spec file was a faithful translation step for Claude, not an inference step. `tests/integration/todos.test.ts` (now 27 cases — 25 against the Todos API + 2 in a sibling CORS describe) lines up directly with the corresponding stories' AC lists by design.
...
| **Updating documentation across the whole repository** | First, Claude did not update the documentation as it made changes. It was then given repeated prompts to review and update test documentation. It started doing this pre-emptively for new changes, but not consistently. Asking it to review all documentation after making a change usually flagged more changes needed. |
...
2. **Run Docker.** No `docker` binary in the sandbox. Claude wrote and statically validated the Dockerfiles + compose YAML (via PyYAML parse) but `docker compose up` happens on my machine. The NFR7 budget check (`scripts/test-compose-up-time.sh`) is wired into CI for that reason — it's the only place the project can verify the compose budget without depending on a local laptop.
3. **Push to GitHub.** Claude can write `LICENSE` and `.github/PULL_REQUEST_TEMPLATE.md` to disk. It cannot authenticate to GitHub or create the repository — that's an intentional and explicit prohibition (no AI auth flows for external accounts) and not a bug to work around. The "ship it to my repo" step is always a manual handoff. Claude gave me a `gh repo create` flow + manual fallback.
4. **See Playwright artifacts when I run the suite.** Failures came to Claude as pasted text. It couldn't view the failure screenshot, the recorded video, or the trace viewer. D13 above is the canonical example of how expensive that gap is — a video would have ruled out Claude's wrong hypothesis instantly.
5. **Generate *correct* time-sensitive UI tests on the first try.** D5 above: fake-timer + microtask scheduling is a category Claude tends to underestimate. A test that compiles and "looks right" is not the same as a test that passes — true generally, doubly true for anything timer-flavoured.
6. **Verify Lighthouse / autocannon improvements in the same pass as the fix.** Claude could write the favicon + `meta description` + `robots.txt` fix, but I had to re-run Lighthouse on my machine to confirm the score moved. Round-trip cost adds up; CI doesn't catch this category because the Lighthouse step is manual.
...
- **Playwright test structure** Claude originally did not use best practice for Playwright locator selection, but after prompting it made the change. Similarly, the tests were initially written with a lot of duplication. After instructing Claude to use a POM (Page Object Model) structure, it made the test files easier to read and removed duplicated lines of code across tests.
...
- **Reviewing documentation.** Documentation could be mostly completed with Claude, however it needed constant prompting to update itself. Afterwards, it needed manual review for tone of voice and reviewing what was missed or not necessary.
